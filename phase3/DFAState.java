...
public class DFAState {
    ...
    public void initialize() {
        ...
        for(int i = 0 ; i < a.length ; i++) {
            char letter = a[i];
            DFAState newState = new DFAState(this.deriv.getDerivative(letter, this.re));
            for(int j = i; j >= 0 ; j--) {
                if(this.trans[j].tree.equals(newState.re)) this.trans[i] = this.trans[j].tree;
                else this.trans[i] = new Tuple(letter, newState);
            }
        }
    }
